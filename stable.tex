...
\begin{proof}[Proof (\kern.1ex\ssf{2}\kern.3ex\boldmath$\IMP$\ssf{1})]
  ...
  \textbf{(\ssf{1}\kern.2ex\boldmath$\IMP$\ssf{2}\kern.1ex)}\ 
   We prove the contrapositive.
   We claim that if there is a branching tree $\bar a$ of height $2^n-2$ then there is a ladder $a_1,\dots,a_{n-1}$ and $b_0,\dots,b_{n-1}$, of length $n$, s.t. $a_1,\dots,        a_{n-1} \in \range(\bar a)$ and $b_0,\dots,b_{n-1} \in \<b_s:s\in{}^{2^n-2}2\>$.
...
\begin{definition}\label{def_approx}
  ...
  If we also have that $\phi(\U\,;b)\subseteq\Aa$, then we say that $\Aa$ is approximable \emph{from below.}
\end{definition}
...
\begin{proof} 
  Suppose for a contradiction that there is a ladder $a_1,\dots,a_n$ and $b_0,\dots,b_n$ of length $n=R(n_1+1,\dots, n_m+1)-1$.
