...
\begin{proof}
  If for all $h\in(n]$ and $k\in[n]$ we have
...
The \emph{2-rank\/} of $\phi(x\,;z)$ is the maximal height of a branching tree for $\phi(x\,;z)$.
...
\begin{proof}[Proof (\kern.1ex\ssf{2}\kern.3ex\boldmath$\IMP$\ssf{1})]
  ...
  We prove the contrapositive.
  We claim that if there is a branching tree of height $2^n+2$ then there is a ladder of length $n$.
  ...
  These are exhaustive by Lemma~\ref{lem_Reamsey_tree}.
...
\begin{proposition}
  ...
  Every set approximable by $\phi(x\,;z)$ is approximable from below by the formula
...
\begin{proof}
  ...
  To prove that $A\subseteq\psi(\U\,;b_0,\dots,b_n)\subseteq\Aa$ for some $b_0,\dots,b_0$.
  ...
  Such a $b_{k+1}$ exists because $\Aa$ is approximable (apply Definition~\ref{def_approx} with $A\cup\{a_1,\dots,a_{k+1}\}$ for $A$).

  We stop at stage $n$.
  Note that we have constructed a ladder of length $n$ for $\neg\phi(x\,;z)$.
  ...
  By Fact~\ref{fact_stability_neg} it is not possible to prolonge this chain, hence
