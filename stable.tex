...
First of all note that Theorem~\ref{thm_stable_definability} remains valid, with the same formula $\psi(x\,;\bar z)$, when $\U$ is replaced by a subset $\U'\subseteq\U$.
In fact, the ladder index of $\phi(x\,;z)$ does not decrease when moving to a subset.

Assume that the $2$-rank of $\psi(x\,;\bar z)$ is infinite.
We claim that, with the appropriate $\U$ and $\V$, there is a branching tree of height $\omega$, say $\{a_s:s\in2^{<\omega}\}$.

Model theoretically, the claim is immediate.
A low-tech combinatorial proof is also not difficult but it requires some handwork which we entrust to the reader.

For $s\in2^\omega$, let $\U_s=\{a_{s\restriction n}:n<\omega\}\subseteq\U$.
Let $\Aa_s=\{a_{s\restriction n}:n<\omega,\ s_n=1\}\subseteq\U_s$.
Then $\Aa_s$ is approximable with respect to the collection $\phi(\U_s\,;b)_{b\in\V}$.
From the theorem above, we obtain $\Aa_s=\psi(\U_s\,;\bar b_s)$ for some  $\bar b_s\in\V^{n\times m}$, where the formula $\psi(x\,;\bar z)$ does non depend on $s$.
...
Therefore $\bar b_s\neq\bar b_{s'}$, hence there is a continuum of tuples $\bar b_s$.
