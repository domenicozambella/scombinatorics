...
In place of requiring the existence of the chain in \ssf{ch}, we could equivalently ask for a pair of tuples $a_1,\dots,a_n\in\U$ and $b_1,\dots,b_n\in\V$ such that
...
\begin{exercise}
  ...
  Let $A\subseteq\U$ be a minimal set such that a chain as in \ssf{ch} obtains for some $b_1,\dots,b_n\in\V$.
...
\begin{proof} Let $A\subseteq\U$ and $b_1,\dots,b_n\in\V$  be such that
  ...
  \ceq{\hfill\phi(A\,;b_1)\ \subset}{\dots}{\subset\ \phi(A\,;b_n)}

  Then
  
  \ceq{\hfill\neg\phi(A\,;b_n)\ \subset}{\dots}{\subset\ \neg\phi(A\,;b_1)}
\end{proof}
...
\ceq{\ssf{2r}\hfill\0}
{\neq}
{\bigcap_{h=0}^{n-1}\neg^{s_h}\,\phi(a_{s\restriction h}\,;\B)}\hfill for all $s\in {}^n2$.

where $\neg^i$, for $i$ a non negative integer, denotes a negation symbol repeated $i$ times.
In other words \ssf{2r} requires the existence of some $\<b_s:s\in{}^n2\>$ in $\B$ such that
    ...
    child {
        node[bag] {\llap{$\neg$}$\phi(a_{\0};z)$}      
            child {
                node[bag] {\llap{$\neg$}$\phi(a_1;z)$}
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi(a_{11};z)$\rlap{ ----- $b_{111}$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi(a_{11};z)$\rlap{ ----- $b_{110}$}}
                       edge from parent
                    }  
                 edge from parent
            }
            child {
                ...
                edge from parent
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi(a_{10};z)$\rlap{ ----- $b_{101}$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi(a_{10};z)$\rlap{ ----- $b_{100}$}}
                       edge from parent
                    }  
                 edge from parent
            }
    ...
    child {
        node[bag] {$\phi(a_{\0};z)$}         
            child {
                node[bag] {\llap{$\neg$}$\phi(a_0;z)$}
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi(a_{01};z)$\rlap{ ----- $b_{011}$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi(a_{01};z)$\rlap{ ----- $b_{010}$}}
                       edge from parent
                    }  
                 edge from parent
            }
            child {
                ...
                edge from parent
                    child {
                       node[bag] {\footnotesize\llap{$\neg$}$\phi(a_{00};z)$\rlap{ ----- $b_{001}$}}
                       edge from parent
                    }    
                    child {
                       node[bag] {\footnotesize$\phi(a_{00};z)$\rlap{ ----- $b_{000}$}}
                       edge from parent
                    }  
                 edge from parent
            } 
...
If a maximal integer $n$ does not exist, we say that the 2-rank of $\B$ is infinite.

\begin{example}
  Clearly ${\rm R}_{\phi(x,z)}(\B)=0$ when $\B$ is empty or when $\phi(\U\,;b)_{b\in\B}$ consists of just one set.
  If there are at least two distinct sets in $\phi(\U\,;b)_{b\in\B}$, then we can always find $a_\0\in\U, b_0,b_1\in\B$ such that $\phi(a_\0,b_0)\niff\phi(a_\0,b_1)$ hence ${\rm R}_{\phi(x,z)}(\U)\ge1$.\QED  
\end{example}

A binary tree $\bar a'=\<a'_r:r\in{}^{<m}2\>$ is a \emph{subtree\/} of $\bar a$ if there is an $\subseteq$-preserving map $f:{}^{<m}2\to{}^{<n}2$ such that $a'_r=a_{fr}$.\llap{$\neg$}
...
\begin{lemma}\label{lem_uno}
  ...
  \ceq{\hfill \psi(x\,;z_1,\dots,z_n)}{=}{\bigwedge_{i=1}^n\phi(x\,;z_i)}
...
\begin{proof}
  ...
  \ceq{\hfill A}{\subseteq}{\bigcap_{i=1}^k\phi(\U\,;b_i)\medrel{\subseteq}\Aa}

  we set $b_{k+1}=\dots=b_n=b_k$ and stop as we already have the required parameters.
  Otherwise pick any element

  \ceq{\hfill a_{k+1}}{\in}{\bigcap_{i=1}^k\phi(\U\,;b_i)\sm\Aa}
  ...
  \ceq{\hfill A}{\subseteq}{\bigcap_{i=1}^n\phi(\U\,;b_i)\medrel{\subseteq}\Aa}
...
\begin{lemma}
  ...
  \\[-1ex]
  \ceq{\hfill \Aa}{=}{\bigcup^n_{i=1}\phi(\U\,;b_i)}
...
\begin{theorem}\label{thm_stable_definability}
  ...
  \ceq{\hfill\psi(x\,;\bar z)}{=}{\bigvee_{j=1}^m\bigwedge_{i=1}^n\phi(x\,;z_{i,j})}
...
\begin{lemma}
  ...
  \ceq{\hfill\psi(x\,;\bar b)}{\IFF}{R_{\phi(x,z)}\big(\big)}
