...
\begin{example}
  Clearly ${\rm R}_{\phi(x,z)}(\B)=0$ when $\B$ is empty or when $\phi(\U\,;b)_{b\in\V}$ consists of just one set.
  If there are at least two distinct sets in $\phi(\U\,;b)_{b\in\V}$, then we can always find $a_\0, b_0,b_1$ such that $\phi(a_\0,b_0)\niff\phi(a_\0,b_1)$ hence ${\rm R}_{\phi(x,z)}(\U)\ge1$.\QED  
\end{example}
