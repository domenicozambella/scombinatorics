...
which we may also write as $\bar a=\<a_r:r\in{}^{<n}2\>$, such that
...
A branching tree $\bar a'=\<a'_r:r\in{}^m2\>$ is a subtree of $\bar a$ if there is an $\subseteq$-preserving map $f:{}^m2\to{}^n2$ such that $a'_r=a_{fr}$.
...
\begin{lemma}
  ...
  Let $k\in[n]$ be given.
  Then, for every red-blue coloring of $\range(\bar a)$,  there is a branching subtree $\bar a'=\<a'_r\, :\, r\in{}^{n'}2\>$ such that either all nodes of $\bar a'$ are red and $n'=k$, or they are all blue and $n'=n-k$.
\end{lemma}
...
\begin{proof}[Proof (\kern.1ex\ssf{2}\kern.3ex\boldmath$\IMP$\ssf{1})]
  ...
  To every pair of elements $b,c\in\V$ we associate a 2-coloring of $\range(\bar a)$ as follows.
  A node $a\in\range(\bar a)$ is colored
  
  \quad red if $\psi(a\,;b,c)=\phi(a\,;b)\vee\phi(a\,;c)$ holds;
  
  \quad blue otherwise, that is, $\neg\psi(a\,;b,c)=\neg\phi(a\,;b)\wedge\neg\phi(a\,;c)$.

  Case 1: for some $b,c$ there is a red subtree $\bar a'$ of height $m+1$.
  Consider the subtree $\bar a'_0=\<a'_{0^\frown r}\,:\,r\in {}^{<\,m}2\>$.
  By induction hypothesis, there are $A\subseteq\range(\bar a'_0)$ and $b_0,c_0,\dots,b_n,c_n$ such that

  \ceq{(1)\hfill\psi(A\,;b_0,c_0)\ \subset}{\dots}{\subset\ \psi(A\,;b_n,c_n)}

  Let $A'=A\cup\{a'_\0\}$ then

  \ceq{(2)\hfill\psi(A'\,;b_0,c_0)\ \subset}{\dots}{\subset\ \psi(A'\,;b_n,c_n)}

  In fact, as $b_0,c_0,\dots,b_n,c_n\in\neg\phi(a'_\0\,;\V)$, these are the same chain as (1).
  Therefore, if we extend the chain on the right with $\psi(A'\,;b,c)=A'$, we obtain the required chain of length $n+1$.
  
  Case 2: for every $b,c$ there is a blue subtree $\bar a'$ of height $m$.
  Choose $b,c$ such that $\phi(a_\0, b)\wedge\neg\phi(a_\0, c)$.
  By the choice of $b$, a blue subtree does not contain $a_\0$.
  Therefore $\bar a'$ is a subtree of $\<a_{i^\frown r}\,:\,r\in {}^{<\,2m}2\>$ for $i$ either $0$ or $1$.
  Apply the induction hypothesis to obtain $A\subseteq\range(\bar a')$ and $b_0,c_0,\dots,b_n,c_n$ such that (1).
  We claim that (2) above holds with $A'=A\cup\{a_\0\}$.
  In fact, if $i=0$ then $b_0,c_0,\dots,b_n,c_n\in\neg\phi(a_\0\,;\V)$ and (2) is the same chain as (1).
  If $i=1$, then $b_0,c_0,\dots,b_n,c_n\in\phi(a_\0\,;\V)$ and (2) is the chain in (1) with all sets augmented by $a_\0$.
  If $i=0$ we can extend the chain on the right with $\psi(A'\,;b,b)=A'$.
  If $i=1$ we can extend the chain on the left with $\psi(A'\,;c,c)=\0$.
  Either way, we obtain the required chain of length $n+1$.
\end{proof}
