...
\begin{proof}[Proof (\kern.1ex\ssf{2}\kern.3ex\boldmath$\IMP$\ssf{1})]
  ...
  In fact, otherwise $n_{\rm 2r}\ge 2^{n_{\rm ld}+1}+2$ and there would exist a latter of length $n_{\rm ld}+1$ which is a contradiction.
  ...
  The following two cases are exhaustive by Lemma~\ref{lem_Reamsey_tree}. Note that we are applying the lemma only to the subtree $\<a_{1^\frown r}:r\in{}^{2m+1}2\>$.
  ...
  \ceq{(1)\hfill\psi(A\,;b_0)\ \subset}{\dots}{\subset\ \psi(A\,;b_n)}

  Let $A'=A\cup\{a'_\0\}$ then

  \ceq{(2)\hfill\psi(A'\,;b_0)\ \subset}{\dots}{\subset\ \psi(A'\,;b_n)}
  ...
  Choose $b$ such that $\neg\phi(a_\0, b)$ and let $\bar a'$ the blue subtree.
