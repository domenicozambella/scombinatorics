...
We call the \vc-dimension of $\phi(x\,;z)^*$ the \emph{dual \vc-dimension\/} or \emph{\vc-codimension\/} of $\phi(x\,;z)$.
...
\begin{exercise}
  ...
  % Hint: Let $\U$ an infinite set and 
  
  % \ceq{\hfill\V}{=}{\bigcup^{m}_{i=0}{\U\choose i}}\hfill where $m=2^{k+1}-1$.

  % The formula $\phi(x\,;z)$ is $x\in z$.
  % As noted above, $\phi(x\,;z)$ has \vc-dimension $2^{k+1}-1$.
  % If we prove that $\phi(x\,;z)^*$ has dimension $k$, we are done (because the dual of the dual is the primal).
  % It is easy to verify that the dimension of $\phi(x\,;z)^*$ is at least $k$.
  % To prove equality, assume for a contradiction that some $B\subseteq\V$ of cardinality $k+1$ is shattered by $\phi(x\,;z)^*$.
  % Then for every $B'\subseteq B$ there is an $a_{B'}\in\U$ such that $B'=\phi(a_{B'}\,;B)$.
  % Or, given that $\phi(x\,;z)$ is $x\in z$,

  % \ceq{\hfill a_{B'}}{\in}{\bigcap_{\ \ b\in B'\ \ }b}

  % \ceq{\hfill a_{B'}}{\notin}{\bigcup_{b\in B\sm B'}b.}

  % Let $A=\{a_{B'}\ :\ \0\neq B'\subseteq B\}$.
  % As $a_{B'}\neq a_{B''}$ when $B'\neq B''$, then $A$ has cardinality $2^{k+1}$.
  % % Let $A=\{a_{\{b\}}\ :\ b\in B\}$.
  % % As $a_{B'}\neq a_{B''}$ when $B'\neq B''$, then $A$ has cardinality $2^{k+1}$.
  % % Note that if $b\neq b'$ then $b\cap A\neq b'\cap A$.
  % % In fact, $a_{\{b\}}\in b\sm b'$ and vice versa $a_{\{b'\}}\in b'\sm b$.
  % % Therefore $|A|=|B|=k+1$.
  % It is easy to check that $\phi(x\,;z)$ shatters also $C=\{b\cap A\ : b\in B\}$.
  % But $C$ 
