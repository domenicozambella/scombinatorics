...
Write $\Indicator_{A$ for the indicator function of $A$.
That is, $\Indicator_As_i=1$ if $s_i\in A$, otherwise $\Indicator_As_i=0$.
Then

\ceq{\hfill \Fr(s,A)}
{=}
{\frac1n\sum^n_{i=1}\Indicator_As_i.}
...
Then $\Indicator_A\smallcirc S_i$ as a Bernoulli random variable with probability of success $\Pr(A)$ and 

\ceq{\hfill \Fr(S,A)}
{=}
{\frac1n\sum^n_{i=1}\Indicator_A\smallcirc S_i}
...
and the same holds for $-\sigma_i\big(\Indicator_b\,s_i -  \Indicator_b\,s'_i\big)$.\QED
\end{document}
