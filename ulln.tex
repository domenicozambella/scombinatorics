...
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
\section{The Uniform Law of Large Numbers (1)}\label{uniform}
...
\begin{proof}
  ...
  \ceq{\hfill\Pr\bigg(\sup_{b\in\V}\Big|\sum_{i=1}^n\sigma_i\,\Indicator_{S_i,b}\Big|\ge\frac{n\epsilon}4\ \big|\  S=s\bigg)}
  {=}
  {\Pr\bigg(\sup_{b\in\V}\Big|\sum_{i=1}^n\sigma_i\,\Indicator_{s_i,b}\Big|\ge\frac{n\epsilon}4\bigg)}
  ...
  Note that, ingeneral, for any real random variables $X_1,\dots,X_m$ we have
  ...
  \ceq{}
  {\le}
  {2\pi_\phi(n)\exp\Big(-\frac{n\epsilon^2}{32}\Big)\ +\ 2\,e^{-n\epsilon^2/2}}

  \ceq{}
  {\le}
  {4\pi_\phi(n)\exp\Big(-\frac{n\epsilon^2}{32}\Big),}
...
\begin{proof}
  ...
  \ceq{\hfill4\;\pi_\phi(n)\,\exp\bigg(-\frac{n\epsilon^2}{32}\bigg)}
  {<}
  {1}

  By Sauer-Shelah Lemma~\ref{lem_sauer}, $\pi_\phi(n)$ grows polynomially.
